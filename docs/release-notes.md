...
- Deployments must leave the `Apply D1 migrations` step enabled in the `Worker Deploy` GitHub Action (or run `npm run migrate:apply` manually) before shifting traffic to ensure the index exists in production.
