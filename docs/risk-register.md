...
| --- | --- | --- | --- | --- | --- |
| Cloudflare Access shim accidentally left on in shared environments, granting admin access without SSO | Low | High | Centralize `withAccess` guard, gate shim behind `ENVIRONMENT` flag, add CI check (`pnpm check:prod-shim`) and deployment checklist updates | `access.dev_shim_used` log outside `development`; CI failures on shim check | Security |
