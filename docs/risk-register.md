...
| Ingest DoS due to per-request D1 rate-limit checks without upstream throttling | Medium | Medium | Introduce KV/Durable Object IP token bucket before hitting D1, reuse ops metrics to detect abuse, and tune heartbeats | Spike in `ingest.rate_limited` logs coupled with high DB CPU, Miniflare load test failures | Platform Ops |
...
**Top 3 Burn Down Now**
1. Implement ingest IP token bucket to remove D1 as the first DoS bottleneck.
