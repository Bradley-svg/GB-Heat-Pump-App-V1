...
| `services/overseas-api` (Cloudflare Workers + D1/KV/R2) | itty-router 5 (MIT), jose 5.9 (MIT), zod 3.23 (MIT), better-sqlite3 8.x (MIT) for tests | MIT | Ed25519 verification now uses native WebCrypto (`utils/ed25519.ts`); ensure `EXPORT_VERIFY_PUBKEY` secrets stay rotated and wrangler 4.45+ is used for latest `undici`. |
