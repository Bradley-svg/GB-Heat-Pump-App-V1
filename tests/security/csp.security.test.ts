...
describe("Content-Security-Policy overrides", () => {
  it("includes configured API and asset origins in CSP directives", async () => {
    const { env, dispose } = await createWorkerEnv({
      ...
      APP_ASSET_BASE: "https://cdn.remote.test/brand",
    });
    ...
    try {
      ...
      expect(html).toContain('href="https://cdn.remote.test/brand/GREENBRO LOGO APP.svg"');
      expect(html).toContain('"assetBase":"https://cdn.remote.test/brand/"');
    } finally {
      dispose();
    }
  });
