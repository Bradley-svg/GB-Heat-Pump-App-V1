...
const EnvSchema = z
  ...
  .superRefine((value: Record<string, unknown>, ctx: z.RefinementCtx) => {
    ...
    const rawIngestFlag =
      typeof value.ALLOW_RAW_INGEST === "string"
        ? value.ALLOW_RAW_INGEST.trim().toLowerCase()
        : "";
    const rawIngestEnabled = rawIngestFlag.length > 0 && LOCAL_FLAG_VALUES.has(rawIngestFlag);
    ...
    if (rawIngestEnabled && !environmentAllowsShim) {
      ctx.addIssue({
        path: ["ALLOW_RAW_INGEST"],
        code: z.ZodIssueCode.custom,
        message: "ALLOW_RAW_INGEST is restricted to local/test ENVIRONMENT values in Mode A deployments",
      });
    }

    if (rawIngestEnabled && !appBaseIsLocal) {
      ctx.addIssue({
        path: ["ALLOW_RAW_INGEST"],
        code: z.ZodIssueCode.custom,
        message:
          "ALLOW_RAW_INGEST may only be enabled when APP_BASE_URL targets a localhost origin for lab use",
      });
    }
