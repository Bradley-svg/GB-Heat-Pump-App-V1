...
function maybeHandlePreflight(req, pathname) {
...
function deriveUserFromClaims(claims) {
  ...
  const rawGroups = Array.isArray(claims.groups) ? claims.groups.map(String) : [];
  const rawRoles = Array.isArray(claims.roles) ? claims.roles.map(String) : rawGroups;
  ...
  const fromClaim = Array.isArray(claims.clientIds) ? claims.clientIds.map(String) : [];
  const fromGroups = rawGroups.filter((g) => g.startsWith("client:")).map((g) => g.slice("client:".length));
  const clientIds = Array.from(/* @__PURE__ */ new Set([...fromClaim, ...fromGroups]));
...
async function handleLatest(req, env, deviceId) {
  ...
  let row;
  if (isAdmin) {
    row = await env.DB.prepare(`SELECT * FROM latest_state WHERE device_id = ?1 LIMIT 1`).bind(deviceId).first();
  } else {
...
async function handleListDevices(req, env) {
  ...
  const items = slice.map((r) => ({
    ...
    map_version: r.map_version ?? null
  }));
...
async function handleHeartbeat(req, env, profileId) {
  ...
  return withCors(json({ ok: true, server_time: (/* @__PURE__ */ new Date()).toISOString() }));
}
...
var app_default = {
  ...
  async scheduled(_event, env, _ctx) {
    ...
    for (const id of ids) {
      ...
      updates.push(
        env.DB.prepare(`UPDATE latest_state SET online=0, updated_at=?2 WHERE device_id=?1`).bind(id, (/* @__PURE__ */ new Date()).toISOString())
      );
      updates.push(
        env.DB.prepare(
          `INSERT INTO ops_metrics (ts, route, status_code, duration_ms, device_id)
           VALUES (?1,'/cron/offline',200,0,?2)`
        ).bind((/* @__PURE__ */ new Date()).toISOString(), id)
      );
    }
