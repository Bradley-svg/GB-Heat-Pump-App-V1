...
function applyLifecycleOptimistic(
  ...
  switch (payload.action) {
    case "acknowledge": {
      ...
      if (payload.assignee) {
        copy.assigned_to = payload.assignee;
      }
      if (payload.comment || payload.assignee) {
        copy.comments = [
          ...copy.comments,
          pendingComment(
            "acknowledge",
            payload.comment ?? null,
            payload.assignee ? { assignee: payload.assignee } : null,
          ),
        ];
      }
      break;
    ...
    case "assign": {
      ...
      copy.comments = [
        ...copy.comments,
        pendingComment("assign", payload.comment ?? null, { assignee: payload.assignee }),
      ];
      break;
    }
    case "resolve": {
      ...
      if (!copy.acknowledged_at) {
        copy.acknowledged_at = now;
      }
      if (payload.comment) {
        copy.comments = [
          ...copy.comments,
          pendingComment("resolve", payload.comment, null),
        ];
      }
      break;
    }
          ...
          : dialog.action === "acknowledge" ?
            {
              action: "acknowledge",
              comment: dialog.comment.trim() || undefined,
              assignee: dialog.assignee?.trim() || undefined,
            }
          : {
              action: "resolve",
              comment: dialog.comment.trim() || undefined,
            ...
            <div className="actions">
              <button
                ...
                disabled={
                  ...
                  (dialog.action === "assign" && !(dialog.assignee ?? "").trim()) ||
                  (dialog.action === "comment" && !dialog.comment.trim())
                }
              >
                Confirm
              </button>
            ...
            return (
              <div key={alert.alert_id} className="card">
                ...
                <div className="stack compact">
                  ...
                  <div className="meta">
                    Assigned to {alert.assigned_to ? alert.assigned_to : "Unassigned"}
                        ...
                        {alert.comments.map((comment) => (
                          <li key={comment.comment_id} className="list-item">
                            <div>
                              ...
                              {comment.metadata && comment.metadata.assignee ? (
                                <div className="meta">
                                  Assignee: {String(comment.metadata.assignee)}
                                </div>
                              ) : null}
                            </div>
                          </li>
